...
def novo_evento(request):
    if request.method == 'GET':
        ...
        cor_secondaria = request.POST.get('cor_secundaria')
...
def gerenciar_evento(request):
    if request.method == 'GET':
        ...
        if nome:
            eventos = eventos.object.filter(nome__contains=nome)
...
def inscrever_evento(request, id):
    ...
    if request.method == 'GET':
        ...
        if request.user in evento.participantes.all():
            messages.add_message(request, constants.ERROR, 'O usuário já está participando')
            return redirect(reverse(f'/eventos/inscrever_evento/{evento}'),)
...
def participantes_evento(request, id):
    ...
    if not evento.criador == request.user:
        raise Http404('Esse evento não é seu, Falo Maluco')
    if request.method == 'GET':
...
def gerar_csv(request, id):
    ...
    with open(path, 'w') as arq:
        ...
        for participante in participantes:
            ...
            writer.writerow(x)
    messages.add_message(request, constants.SUCCESS, 'CSV gerado com sucesso')
    return redirect(f'/media/{token}')

def certificados_evento(request, id):
    evento = get_object_or_404(Evento, id=id)
    if not evento.criador == request.user:
        raise Http404('Esse evento não é seu, Falô Maluco')
    
    if request.method == 'GET':
        qtd_certificados = evento.participantes.all().count() - Certificado.objects.filter(evento=evento).count()
        return render(request, 'certificado_evento.html',{'evento': evento, 'qtd_certificados': qtd_certificados})
...
def procurar_certificado(request, id):
    ...
    certificado = Certificado.objects.filter(participantes__email=email, evento=evento).first()
    if not certificado:
        messages.add_message(request, constants.ERROR, 'Certificado não Gerado')
        return redirect(reverse('certificados_evento', kwargs={'id': evento.id}))
